...
from mini_smolagents.config import TRUNC_MEDIUM
...
def test_profile_roundtrip(tmp_path):
    ...
    assert loaded.data["constraints"] == ["不用 emoji"]


def test_load_missing_returns_empty(tmp_path):
    p = Profile.load(str(tmp_path / "nonexistent"))
    assert p.data["name"] == ""
    assert p.data["feedback"] == []
...
def test_load_wrong_types_reset(tmp_path):
    d = tmp_path / "user_profile.json"
    d.write_text(json.dumps({"feedback": "not-a-list", "name": "小明"}), encoding="utf-8")
    p = Profile.load(str(tmp_path))
    assert p.data["feedback"] == []
    assert p.data["name"] == "小明"


def test_load_ignores_old_facts(tmp_path):
    d = tmp_path / "user_profile.json"
    d.write_text(json.dumps({"name": "小明", "facts": ["旧事实"]}), encoding="utf-8")
    p = Profile.load(str(tmp_path))
    assert p.data["name"] == "小明"
    assert "facts" not in p.data
...
def test_item_truncated():
    p = Profile()
    p.append_constraints(["x" * (TRUNC_MEDIUM + 100)])
    assert len(p.data["constraints"][0]) == TRUNC_MEDIUM


def test_size_limit_rejects():
    p = Profile()
    big = ["x" * TRUNC_MEDIUM] * 40
    assert p.append_feedback(big) is False
    assert p.data["feedback"] == []


def test_empty_items_noop():
    p = Profile()
    assert p.append_feedback([]) is False
    assert p.append_feedback("") is False
    assert p.data["feedback"] == []
